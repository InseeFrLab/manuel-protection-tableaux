...
hiérarchique puisqu'il existent des emboîtements entre les différents
niveaux.
...
représenter de telle sorte que Tau-Argus sache comprendre les différents
...
0.  Analyse des caractéristiques des tableaux (étape préparatoire)
    ...
    -   Vérifier les hiérarchies et les totaux Il est important de
        détecter la présence éventuelles de variables hiérarchiques dans
...
-   **list_explanatory_vars** : liste des noms des variables de
    croisements présentes dans la liste des table à protéger dans la
    table;
...
-   **value** : le nom de colonne de la variable de réponse dans les
    tableau, il DOIT être identique pour chaque table (ex :
...
-   **cost_var** : variable numeric qui permet de changer la coût de
...
le secret secondaire. Ainsi, certains coûts ne seront pas définis. Ce n'est pas un problème,
